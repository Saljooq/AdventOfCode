...
func main() {
	...
	for i, _ := range start_y_coors {
		...
		if isSurrounder, xloc, yloc := isSurroundedBySymbol(y, x0, x1, arr); isSurrounder {
			if arr[yloc][xloc] == '*' {
				var res int
				if x1+1 >= len(arr[y]) {
				...
				existing, already := store[[2]int{yloc, xloc}]
				if already {
					if existing >= 0 {
						store[[2]int{yloc, xloc}] = 0
					} else {
						store[[2]int{yloc, xloc}] = -1 * res * existing
					}
				} else {
					store[[2]int{yloc, xloc}] = -1 * res
				}
			}
		}
	}
...
func isSurroundedBySymbol(y, x0, x1 int, store [][]rune) (bool, int, int) {
